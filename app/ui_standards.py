...
def app_stylesheet(zoom_percent: int = 100) -> str:
    body=scaled(FONTS["body_size"],zoom_percent); small=scaled(FONTS["small_size"],zoom_percent)
    section=scaled(FONTS["section_size"],zoom_percent); title=scaled(FONTS["title_size"],zoom_percent); hero=scaled(FONTS["hero_size"],zoom_percent)
    radius=scaled(5,zoom_percent); pad=scaled(7,zoom_percent)
    return f"""
...
def apply_global_style(widget: QWidget, zoom_percent: int = 100) -> None:
    widget.setStyleSheet(app_stylesheet(zoom_percent)); font=QFont(widget.font()); font.setPointSize(scaled(FONTS["body_size"],zoom_percent)); widget.setFont(font)
def configure_application(app: QApplication, zoom_percent: int = 100) -> None:
    app.setStyle("Fusion"); app.setStyleSheet(app_stylesheet(zoom_percent))
