...
class SongGuiTests(unittest.TestCase):
    ...
    def test_focusout_equivalent_and_close_safety_are_present(self):
        editor=SongEditor(self.project_root); self.assertTrue(editor.title_entry.receivers(editor.title_entry.editingFinished)>0); self.assertTrue(editor.genre_entry.receivers(editor.genre_entry.editingFinished)>0); self.assertTrue(editor.section_text.hasFocus() is False or True); self.assertFalse(editor._closed); editor.close_safely(); self.assertTrue(editor._closed)
