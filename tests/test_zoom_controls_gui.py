...
class FakeWheel:
    def __init__(self, delta): self._delta, self.accepted = delta, False
...
class ZoomControlsGuiTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.dashboard.show(); self.app.processEvents()
    def tearDown(self):
    ...
    def test_zoom_bounds_and_reset(self):
        self.dashboard.set_zoom(200); self.dashboard._step_zoom(1)
    ...
    def test_font_buttons_exist(self):
        texts = [button.text() for button in self.dashboard.findChildren(type(self.dashboard.findChildren.__self__) if False else __import__('PySide6.QtWidgets', fromlist=['QPushButton']).QPushButton)]
        self.assertIn("A−", texts); self.assertIn("A+", texts)
