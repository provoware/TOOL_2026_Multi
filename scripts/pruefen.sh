...
runtime_dateien=(MANIFEST.json ANLEITUNG_LAIEN.md requirements.txt schnellstart.sh app/main.py app/event_log.py app/regression.py app/redaction.py app/log_maintenance.py app/texts.py app/ui.py app/ui_standards.py scripts/pruefen.sh scripts/start_status.py scripts/process_watch.py scripts/diagnosepaket.py texte/registry.json)
for datei in "${runtime_dateien[@]}"; do pruefe_datei "$datei"; done
if [[ "$MODE" == "--full" ]]; then
  entwickler_dateien=(README.md AGENTS.md TODO.md CHANGELOG.md scripts/veroeffentlichen.py scripts/iteration_restore.py scripts/backup_erstellen.sh tests/test_event_management.py tests/test_release_builder.py tests/test_security_watch.py tests/test_restore.py tests/test_diagnostics_logging.py tests/regression_registry.json agents/INFO_DATEIEN_AGENT.md)
...
if [[ "$MODE" == "--full" ]]; then
  ...
  printf '\nPrüfe Rückfall-, Release-, Sicherheits-, Restore- und Diagnosebausteine …\n'
  python3 -m unittest tests.test_event_management tests.test_release_builder tests.test_security_watch tests.test_restore tests.test_diagnostics_logging || fehler=1
